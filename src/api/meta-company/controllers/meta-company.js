...
/**
 * meta-company controller
 */

const { createCoreController } = require('@strapi/strapi').factories;

module.exports = createCoreController('api::meta-company.meta-company',
({ strapi }) => ({
    ...
    async stripeConnect(ctx) {
        ...
        const meta = await strapi.db.query("api::meta-company.meta-company").findOne({ where: { usuario: user.id } });

        if (!meta) {

            // le creamos	una meta

            const meta = await strapi.db.query("api::meta-company.meta-company").create({ data: { usuario: user.id } });
